...
module.exports = {
  ...
  sub: entry => {
    ...
    return {
      ...
      recipientName: utils.displayName(entry.data['msg-param-recipient-user-name'],
  ...
  tip: entry => {
    ...
    return {
      ...
      timestamp: getTimestamp(entry.data.created_at),
      message: parseEmojiOne(processEmotes(message, emotes)),
      amount: `${getSymbolFromCurrency(entry.data.currency)}${entry.data.amount}`,
...
function parseCheerActions(message, actions) {
  ...
  while ((result = reg.exec(message))) {
    const tiers = actions.find(x => x.prefix.toLowerCase() === result[1].toLowerCase()).tiers
...
function processEmotes(message, emotes) {
  ...
  emotes.forEach(emote => {
    ...
    chars[emote.start] = `<img title="${emote.code}" class="emote" src="${emote.url}">`;
  });
