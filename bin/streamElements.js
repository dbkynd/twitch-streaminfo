...
module.exports = io => {
  ...
  streamElements.on('connect', () => {
    ...
    streamElements.emit('authenticate', { method: 'jwt', token: config.streamElements.token });
  });
  ...
  streamElements.on('event', event => {
    ...
    entry.save()
      ...
      .catch(log.error);
    mongo.Latest.findOneAndUpdate({ name: 'tip' }, { name: 'tip', data }, { upsert: true }).catch(log.error);
  });
};
