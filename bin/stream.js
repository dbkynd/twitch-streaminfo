...
function pollStreamStatus() {
  ...
  request.get(`https://tmi.twitch.tv/hosts?include_logins=1&host=${config.twitch.id}`)
    .then(hostResult => {
      ...
      request.get(`https://api.twitch.tv/helix/streams?user_id=${liveStatus.targetId || config.twitch.id}&type=live`)
        ...
        .then(streamResult => {
          ...
          if (stream) {
            ...
            if (liveStatus.timeStopped) {
              if (moment() > moment(liveStatus.timeStopped).add(config.minutesBeforeConsideredOffline, 'm')) {
...
module.exports = io => {
  ...
  function checkFrontPage() {
    return new Promise((resole, reject) => {
      request.get(`https://api.twitch.tv/kraken/streams/featured` +
        ...
        .then(response => {
          ...
          const existsOnFrontPage = Boolean(response.body.featured
            .find(x => x.stream.channel._id.toString() === config.twitch.id.toString() && x.priority <= 6));
