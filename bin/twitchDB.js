...
function getUsers(ids) {
  return new Promise(async (resolve, reject) => {
    ...
    const usersInDatabase = await mongo.TwitchUsers.find({
      twitchId: { $in: ids },
    })
    ...
    // Create new docs
    const docs = twitchResults.map(
      (x) => new mongo.TwitchUsers({ twitchId: x.id, data: x })
    )
    // Save new docs
    const saved =
      docs.length > 0
        ? await mongo.TwitchUsers.insertMany(docs, { new: true })
        : []
...
function getGames(ids) {
  return new Promise(async (resolve, reject) => {
    ...
    const gamesInDatabase = await mongo.TwitchGames.find({
      twitchId: { $in: ids },
    })
    ...
    const gamesToLookup = ids.filter(
      (id) =>
        gamesInDatabaseIdsArray.indexOf(id) === -1 ||
        gamesInDatabase.find((x) => x.twitchId === id).expires <= time
    )
    if (gamesToLookup.length > 0)
      debug(`${gamesToLookup.length} games do not exist or are expired`)
    // Get missing game data from twitch
    const twitchResults =
      gamesToLookup.length > 0
        ? utils.get(['body', 'data'], await twitchAPI.getGames(gamesToLookup))
        : []
    if (twitchResults === null) reject()
    if (gamesToLookup.length > 0)
      debug(`Got twitch data on ${twitchResults.length} games`)
    // Delete expired docs from database
    const expiredDocs = gamesInDatabase
      .filter((x) => x.expires <= time)
      .map((x) => x.twitchId)
    if (expiredDocs.length > 0) {
    ...
    // Create new docs
    const docs = twitchResults.map(
      (x) => new mongo.TwitchGames({ twitchId: x.id, data: x })
    )
    // Save new docs
    const saved =
      docs.length > 0
        ? await mongo.TwitchGames.insertMany(docs, { new: true })
        : []
