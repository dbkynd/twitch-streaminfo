...
function getSubs() {
  return new Promise(resolve => {
    ...
    const subs = [];
    twitchAPI.getSubscriptions(0, 1)
      .then(async response => {
        ...
        fs.writeFile('./subscribers', JSON.stringify(subscribers), { encoding: 'utf8' }, err => {
          if (err) {
...
function delayedGetSub(offset) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      twitchAPI.getSubscriptions(offset, 100)
...
async function checkLive() {
  ...
  for (let i = 0; i < subscribers.length; i++) {
    ...
    if (chunk && chunk.data.length > 0) {
      ...
      live.push(chunk.data.map(stream => {
        stream.game = utils.get(['data'], games.find(game => game.twitchId === stream.game_id));
        stream.user = utils.get(['data'], users.find(user => user.twitchId === stream.user_id));
        return stream;
      }));
    }
  }
  currentlyLive = live.reduce((prev, curr) => prev.concat(curr), [])
    .filter(x => x.game && x.viewer_count > 0 && x.user.id !== config.twitch.id);
...
function delayedGetLiveStreams(chunk, index) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      twitchAPI.getLiveStreams(chunk)
        .then(response => {
