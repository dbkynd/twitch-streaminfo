...
function saveHighlight(query, user) {
  return new Promise(async (resolve, reject) => {
    ...
    const videos = utils.get(['body', 'data'], await twitchAPI.getVideos(body.user_id));
    const recording = videos ? videos.find(x => x.created_at >= body.started_at) : null;
    const streamData = await getStreamData(body);
    const link = recording ? `${recording.url}?t=${streamData.clipStart}` : 'Unable to locate past broadcast URL';
    const data = {
      body,
      streamData,
      notes: `${user}: ${query}`,
    ...
    addLine(data).then(() => {
      ...
      resolve(`That ${hosted}'${streamData.game.name}' moment was saved. PogChamp${link ? ` ${link}` : ''}`);
    }).catch(reject);
  });
}
...
async function getStreamData(body) {
  ...
  const streamStart = moment(body.started_at).tz('America/Los_Angeles').format('YYYY-MM-DD h:mma z');
  const d = moment.duration(moment() - moment(body.started_at)
  ...
  my_sheet2.useServiceAccountAuth(my_sheet_creds, authErr => {
    ...
    my_sheet2.getInfo((infoErr, info) => {
      ...
      sheet.getCells({
        ...
        const subscribers = subscribersChunksArray.reduce((prev, curr) => prev.concat(curr), []);
        const data = {
          prime: 0,
          tier1000: 0,
          tier2000: 0,
        ...
        cells[0].value = moment().tz('America/Los_Angeles').format('YYYY-MM-DD h:mma z');
...
function saveNote(data) {
  return new Promise((resolve, reject) => {
    my_sheet3.useServiceAccountAuth(my_sheet_creds, error => {
      if (error) return reject(error);
      my_sheet3.addRow(1, {
        Date: moment(data.date).tz('America/Los_Angeles').format('MM-DD-YYYY h:mma z'),
        Note: data.note,
      }, err => {
        if (err) return reject(err);
        resolve();
