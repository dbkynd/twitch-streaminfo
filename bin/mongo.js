...
// Tips
const Tips = mongoose.model('tips', new mongoose.Schema({
  data: Object,
  cleared: Boolean,
}));
...
// Cheers
const Cheers = mongoose.model('cheers', new mongoose.Schema({
  data: Object,
  cleared: Boolean,
}));

// Hosts
const Hosts = mongoose.model('hosts', new mongoose.Schema({
  data: Object,
  cleared: Boolean,
  isRaid: Boolean,
}));
...
// Twitch User Data
const TwitchUsers = mongoose.model('twitch_users', new mongoose.Schema({
...
// Twitch Game Data
const TwitchGames = mongoose.model('twitch_games', new mongoose.Schema({
  twitchId: { type: String, index: true },
  data: Object,
  expires: { type: Date, default: moment().add(7, 'd') },
}));
...
const Reported = mongoose.model('twitch_reported_users', new mongoose.Schema({
...
const Counts = mongoose.model('counts', new mongoose.Schema({
  name: String,
  count: Number,
}));
...
// Latest Events
const Latest = mongoose.model('latests', new mongoose.Schema({
  name: String,
  data: Object,
}));
