...
router.get('/getTips', (req, res, next) => { // eslint-disable-line no-unused-vars
  req.db.Tips.find({}).sort('-data.created_at')
    .then(results => {
...
router.get('/getSubs', (req, res, next) => { // eslint-disable-line no-unused-vars
  req.db.Subscriptions.find({}).sort('-data.time')
    .then(results => {
...
router.get('/getCheers', (req, res, next) => { // eslint-disable-line no-unused-vars
  req.db.Cheers.find({}).sort('-data.time')
    .then(results => {
...
router.get('/getHosts', (req, res, next) => { // eslint-disable-line no-unused-vars
  req.db.Hosts.find({}).sort('-data.created_at')
    .then(results => {
...
router.post('/restart', (req, res, next) => { // eslint-disable-line no-unused-vars
  if (config.allowedToEdit.indexOf(req.user) === -1) return res.sendStatus(403);
  res.sendStatus(200);
  process.exit(0);
});

router.post('/saveNote', (req, res, next) => { // eslint-disable-line no-unused-vars
  Promise.all([
...
module.exports = io => {
  router.post('/clearItem', (req, res, next) => { // eslint-disable-line no-unused-vars
    if (config.allowedToEdit.indexOf(req.user) === -1) return res.sendStatus(403);
    ...
    model.findByIdAndUpdate(req.body.id, { cleared: true }, { upsert: true })
      .then(() => {
        res.sendStatus(200);
