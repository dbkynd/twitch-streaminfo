...
app.use('/auth/not_authorized', (req, res, next) => { // eslint-disable-line no-unused-vars
...
app.use((req, res, next) => {
  ...
  if (config.allowedToView.indexOf(req.user) === -1) return res.redirect('/auth/not_authorized');
  return next();
});

// Force refresh clients if requested by DBKynd
app.get('/forceReload', (req, res, next) => { // eslint-disable-line no-unused-vars
...
function verifySignature(req, res, next) {
  const signature = `sha256=${crypto.createHmac('sha256', webhooks.secret)
